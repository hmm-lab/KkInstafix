...
# ── URL store: hash -> original URL (avoids Telegram 64-byte callback limit) ─
URL_STORE: dict = {}
...
# ── Keyboard (uses URL hash, not raw URL, to stay under 64-byte limit) ────────

def make_keyboard(platform, original_url):
    url_hash = store_url(original_url)
    emoji    = PLATFORM_EMOJI.get(platform, "?")
    buttons  = [
        InlineKeyboardButton(
            f"{emoji} {key}",
            callback_data=f"{platform}|{key}|{url_hash}"
        )
        for key in PROVIDERS[platform]["options"]
    ]
...
async def handle_message(update, context):
    ...
    keyboard = (
        ...
        if detected_platform and orig_url else None
    )
    ...
    try:
        ...
        await context.bot.send_message(
            ...
            reply_markup=keyboard, link_preview_options=preview,
        )
    except Exception:
        await msg.reply_text(
            post_text, reply_markup=keyboard, link_preview_options=preview,
        )
...
async def handle_callback(update, context):
    ...
    try:
        platform, provider_key, url_hash = query.data.split("|", 2)
    except ValueError:
        return

    original_url = recall_url(url_hash)
    if not original_url:
        await query.answer(
            "Session expired - please resend the link.", show_alert=True
        )
        return
